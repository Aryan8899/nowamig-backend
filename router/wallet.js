  ...
  try {
    ...
    records.forEach((record) => {
      ...
      allocations[address] = {
        ...
        bigbait: parseFloat(record.bigbait_amount),
      };
    });
    return allocations;
  } catch (error) {
...
const getUserAllocation = (userAddress, tokenType) => {
  ...
  return tokenType.toUpperCase() === "TARAL"
    ? allocation.taral
    : tokenType.toUpperCase() === "RVLNG"
    ? allocation.rvlng
    : tokenType.toUpperCase() === "BIGBAIT"
    ? allocation.bigbait
    : null;
};
...
router.post("/claim", async (req, res) => {
  ...
  try {
    ...
    // Validate token type
    if (!["TARAL", "RVLNG", "BIGBAIT"].includes(tokenType.toUpperCase())) {
      return res.status(400).json({
        success: false,
        error: "Invalid token type. Must be TARAL, RVLNG, or BIGBAIT",
      });
    }
   ...
   VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    ...
    const claimed = { taral: 0, rvlng: 0, bigbait: 0 };
    ...
    claimedAmounts.rows.forEach((row) => {
      ...
      else if (row.token_type === "BIGBAIT")
        claimed.bigbait = parseFloat(row.total_amount);
    });
    ...
    res.status(200).json({
      ...
      data: {
        ...
        userTotals: {
          ...
          bigbaitClaimed: claimed.bigbait,
...
router.get("/allocations/:address", async (req, res) => {
  try {
    ...
    claimedTokens.rows.forEach((claim) => {
      ...
      else if (claim.token_type === "BIGBAIT")
        claimed.bigbait += parseFloat(claim.amount);
    });
    ...
    const bigbaitRemaining = Math.max(0, allocation.bigbait - claimed.bigbait);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        allocations: {
          ...
          bigbait: {
            allocated: allocation.bigbait,
            claimed: claimed.bigbait,
            remaining: bigbaitRemaining,
            canClaim: bigbaitRemaining > 0,
          },
        },
      },
    });
...
router.get("/claims/:address", async (req, res) => {
  try {
    ...
    // Get user summary
    const userQuery = await query("SELECT * FROM users WHERE address = $1", [
      address,
    ]);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        summary: {
          ...
          bigbaitClaimed: parseFloat(userWallet.bigbait_claimed) || 0,
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const bigbaitTotal = await query(
      "SELECT COALESCE(SUM(amount), 0) as total FROM claims WHERE token_type = 'BIGBAIT'"
    );

    const totalbigbaitAllocated = Object.values(WALLET_ALLOCATIONS).reduce(
      (sum, allocation) => sum + allocation.bigbait,
      0
    );

    // Calculate total allocations from CSV
    const totalTaralAllocated = Object.values(WALLET_ALLOCATIONS).reduce(
      (sum, allocation) => sum + allocation.taral,
      0
    );
    const totalRvlngAllocated = Object.values(WALLET_ALLOCATIONS).reduce(
      (sum, allocation) => sum + allocation.rvlng,
      0
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        totalbigbaitClaimed: parseFloat(bigbaitTotal.rows[0].total),
        totalbigbaitAllocated: totalbigbaitAllocated,
